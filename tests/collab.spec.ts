...
test('a shared hole becomes readable to another user; private stays hidden', async ({ users }) => {
  ...
  await owner.page.waitForURL(/\/hole\/.+/, { timeout: 15000 })
  ...
  await expect(other.page.getByText("This hole isn't here")).toBeVisible({ timeout: 15000 })
  ...
  try {
    ...
    await owner.page.getByRole('button', { name: /delete hole/i }).last().click()
